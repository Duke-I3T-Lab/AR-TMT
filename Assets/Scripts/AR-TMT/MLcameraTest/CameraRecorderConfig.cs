...
namespace MagicLeap.Examples
{
    ...
    public class CameraRecorder
    {
        ...
        private void MediaRecorderOnTrackInfo(MLMediaRecorder.OnTrackInfoData info)
        {
            ...
            Debug.Log($"[Info] Type: {info.Info}, Extra: {info.Extra}");

        // Handle specific info types
            switch (info.Info)
            {
                case MLMediaRecorder.Info.MaxDurationReached:
                    Debug.Log("Maximum duration reached.");
                    break;

                case MLMediaRecorder.Info.MaxFileSizeReached:
                    Debug.Log("Maximum file size reached.");
                    break;

                case MLMediaRecorder.Info.TrackProgressInTime:
                    Debug.Log($"Progress in time: {info.Extra} ms");
                    break;

                case MLMediaRecorder.Info.TrackEncodedFrames:
                    Debug.Log($"Encoded frames: {info.Extra}");
                    break;

                case MLMediaRecorder.Info.TrackDataKBytes:
                    Debug.Log($"Recorded data: {info.Extra} KB");
                    break;
            }
        }
        ...
        {
            OnInfo?.Invoke(info);
            Debug.Log($"[Track Info] Type: {info.Info}, Extra: {info.Extra}");

                // Handle specific track info types
                if (info.Info == MLMediaRecorder.Info.TrackDurationMs)
                {
                    Debug.Log($"Track Duration: {info.Extra} ms");
                }
        }
