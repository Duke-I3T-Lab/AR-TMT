...
public class TargetGenerator : MonoBehaviour
{
    ...
    [Tooltip("Air Generation R max.")]
    public float generation_r_max = 3f; // Default value, adjustable in the Inspector
    [Tooltip("Air Generation R min")]
    public float generation_r_min = 1f; // Default value, adjustable in the Inspector
    [Tooltip("Air Generation Angle")]
    public float generation_angle = 150f; // Default value, adjustable in the Inspector
    [Tooltip("Air Generation y range")]
    ...
    // Clutter
    public float minAngular_clutterX = 6f; // Ensures moderate spacing for distractors (in degrees)
    ...
    private bool ValidateAndPrepareWalls(out List<SharedInfomanager.WallData> savedWalls, out float minDistanceBetweenTargets, out float minHeightOffset, float targetsize)
    {
        savedWalls = SharedInfomanager.SavedWalls;
        if (savedWalls == null || savedWalls.Count == 0)
        {
            Debug.LogError("No saved walls available for target placement!");
            minDistanceBetweenTargets = 0f;
            minHeightOffset = 0f;
            return false;
        }

        minDistanceBetweenTargets = targetsize * 1.5f; // Use the passed targetlength for spacing
        minHeightOffset = 0.2f; // Minimum height offset to avoid low positions
        return true;
    }
    ...
    public void GenerateTargets(bool useWalls = false)
    {
        ...
        float targetlength= Mathf.Max(targetSize.x, targetSize.z, targetSize.y); // Use the maximum size for spacing

        if (useWalls)
        {
            Debug.Log("Placing targets on saved walls...");

            if (!ValidateAndPrepareWalls(out List<SharedInfomanager.WallData> savedWalls, out float minDistanceBetweenTargets, out float minHeightOffset, targetlength))
            {
                return;
            }

            // Initialize bins for each wall
            int[] bins = new int[savedWalls.Count];
            // Get the camera's position and forward direction
            Vector3 camPosition = Camera.main.transform.position;
            Vector3 camForward = Camera.main.transform.forward;

            // Compute the camera's right vector based on a fixed up direction
            Vector3 camRight = Vector3.Cross(Vector3.up, camForward).normalized;

            // Reorder walls based on their relative horizontal position compared to the camera's position
            savedWalls = savedWalls.OrderBy(wall => Vector3.Dot((wall.Center - camPosition).normalized, camRight)).ToList();

            switch (SharedInfomanager.Instance.currentGeneration)
            {
                case 1:
                    bins = new int[] { 4, 4, 3, 3, 3, 3, 2, 3 }; // Updated bin distribution for generation 1
                    break;
                case 2:
                    bins = new int[] { 3, 2, 4, 4, 3, 3, 3, 3 }; // Updated bin distribution for generation 2
                    break;
                case 3:
                    bins = new int[] { 3, 3, 2, 3, 4, 3, 3, 4 }; // Updated bin distribution for generation 3
                    break;
                case 4:
                    bins = new int[] { 3, 4, 3, 3, 3, 3, 2, 4 }; // Updated bin distribution for generation 4
                    break;
                case 5:
                    bins = new int[] { 3, 3, 4, 3, 3, 4, 3, 2 }; // Updated bin distribution for generation 5
                    break;
            }

            int lastWallIndex = -1; // Track the last wall used

            for (int i = 0; i < numberOfTargetsToGenerate; i++)
            {
                SharedInfomanager.WallData selectedWall;
                int wallIndex;

                // Extract a bin to place a target
                List<int> availableWalls = bins
                    .Select((bin, index) => new { bin, index })
                    .Where(x => x.bin > 0 && (bins.Count(b => b > 0) == 1 || x.index != lastWallIndex))
                    .Select(x => x.index)
                    .ToList();

                if (availableWalls.Count == 0)
                {
                    Debug.LogWarning($"No valid wall found for target {i + 1}. Skipping...");
                    continue;
                }

                // Randomly select a wall from the available walls
                wallIndex = availableWalls[Random.Range(0, availableWalls.Count)];

                bins[wallIndex]--; // Decrease the bin count for the selected wall
                selectedWall = savedWalls[wallIndex];
                lastWallIndex = wallIndex; // Update the last wall used
                ...
                for (int attempt = 0; attempt < maxAttempts; attempt++)
                {
                    ...
                    float randomX = Random.Range(-selectedWall.Width / 2 + targetlength, selectedWall.Width / 2 - targetlength);
                    float randomY = Random.Range(-selectedWall.Height / 2 + targetlength + minHeightOffset, selectedWall.Height / 2 - targetlength);
                    ...
                    bool tooClose = false;
                    foreach (var existingObject in targets.Concat(distractors))
                    {
                        if (Vector3.Distance(existingObject.transform.position, validPosition) < minDistanceBetweenTargets)
                        {
                            tooClose = true;
                            break;
                        }
        ...
        if (SharedInfomanager.Instance.currentGeneration == 3)
        {
            ...
            GenerateTopdowndistractors(useWalls, markerPose, markerSize, 20, false);
        ...
        {
            GenerateTopdowndistractors(useWalls, markerPose, markerSize, 20, true);
        ...
        {
            GenerateBottomUpDistractors(useWalls, markerPose, markerSize, 20);
        }
    ...
    public void GenerateTopdowndistractors(bool useWalls, Pose markerPose, float markerSize, int numberOfDistractors, bool strong, bool cluttering = false)
    {
        ...
        if (!ValidateAndPrepareWalls(out List<SharedInfomanager.WallData> savedWalls, out float minDistanceBetweenTargets, out float minHeightOffset, targetlength))
        {
            Debug.LogError("Failed to validate walls for distractor placement.");
            return;
        }
        ...
        while (distractors.Count < numberOfDistractors && attempts < maxAttempts)
        {
            ...
            if (useWalls)
            {
                ...
                if (!foundValidPosition)
                {
                    Debug.LogWarning($"Failed to find a valid position for distractor {distractors.Count + 1}. Skipping...");
                    continue;
                }
            }
            else
            {
                validPosition = GetValidPosition_extra(userPos, cluttering, MaxRadius_clutter, minSpacing_clutter, markerPose, GetTargetSize(topdown_weak_DistractorPrefab));
            }

            distractor = Instantiate(topdown_weak_DistractorPrefab, validPosition, Quaternion.identity);
            if (distractor == null)
            {
                Debug.LogError($"Failed to instantiate distractor prefab for {distractors.Count + 1}.");
                continue;
            }

            distractor.name = $"GeneratedDistractor {distractors.Count + 1}";
            ...
            if (useWalls)
            {
                ...
                Quaternion targetRotation = Quaternion.LookRotation(wallNormal, Vector3.up);
                distractor.transform.rotation = targetRotation * Quaternion.Euler(90, 0, 0); // Adjust based on prefab orientation
            ...
            if (strong)
            {
                ...
                if (textChild != null)
                {
                    ...
                    textInstance.localPosition = new Vector3(0, 1, 0); // Set local position (X = 0, Y = 2, Z = 0)
            ...
            Debug.Log($"Placed distractor {distractors.Count} on wall at {validPosition}");
    ...
    void GenerateBottomUpDistractors(bool useWalls, Pose markerPose, float markerSize, int numberOfDistractors, bool cluttering = false)
    {
        ...
        while (distractors.Count < numberOfDistractors && attempts < maxAttempts)
        {
            ...
            if (useWalls)
            {
                ...
                if (!foundValidPosition)
                {
                    Debug.LogWarning($"Failed to find a valid position for target. Skipping...");
                    continue;
                }
            }
            else
            ...
            distractor = Instantiate(distractorPrefab, validPosition, Quaternion.identity);
            ...
            if (renderer != null)
            {
                Color[] distractorColors = {
                    new Color(1f, 0.0f, 0.0f),    // Red
                    new Color(1f, 0.5f, 0.0f),    // Orange
                    new Color(1f, 1f, 0.0f),      // Yellow
                    new Color(0.0f, 1f, 0.0f),    // Green
                    new Color(0.0f, 1f, 1f),      // Cyan
                    new Color(0.0f, 0.0f, 1f),    // Blue
                    new Color(0.5f, 0.0f, 1f),    // Violet
                    new Color(1f, 0.0f, 1f)       // Magenta
                };
    ...
    private Vector3 GetValidPosition_extra(Vector3 userPos, bool cluttering, float clutterRadius, float minSpacing, Pose markerPose, Vector3 distractorSize)
    {
        ...
        // 1. cluttering : it choses random anchor target and place distractors near it.
        // 2. non-cluttering : it chooses random position in the space and place distractors there.
