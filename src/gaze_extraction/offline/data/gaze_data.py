...
class CPRARGazeData(GazeData):
    ...
    def load_data(self, file_path: str, is_demo=False) -> None:
        ...
        # self.left_pupil_diameter = data["LeftPupilDiameter"].to_numpy()
        # self.right_pupil_diameter = data["RightPupilDiameter"].to_numpy()
    ...
    def preprocess_target_data(self, df, binary=True):
        ...
        # Apply mode filter to smooth the target data
        # df["GazeTarget"] = mode_filter(df["GazeTarget"])
    ...
    def interpolate_missing_gaze_data(self, df):
        ...
        for gap in large_gaps:
            ...
            #     new_row["RealTime"],
            #     " between ",
            #     df["RealTime"].iloc[gap],
            #     " and ",
            #     df["RealTime"].iloc[gap + 1],
            # )
            ...
            for col in df.columns:
                if col in average_columns:
                    ...
                    # use the most column value in the neighboring 5 rows
                    # print(df[col].iloc[gap-2:gap+3])
    ...
    # def split_columns_and_save(self, feature_df, col):
    #     feature_df[[f"{col}_x", f"{col}_y", f"{col}_z"]] = (
    #         feature_df[col].str.strip("()").str.split(r"\|\s*", expand=True)
    #     )
    ...
    def slice_data_with_config(self, config="before_activation", duration=30):
        if config == "Before_Confirmation":

            last_index = np.where(self.situation == self.confirm_index)[0][0]
            # find the index that is "duration" seconds before the first index
        ...
        if config == "From_Start":
            ...
            else:  # if not specified, use before confirmation if provided duratino < 0
        ...
        if config == "Before_Recognition":
            ...
            if target_time < self.start_timestamp[0]:
                ...
                first_index = np.argmin(np.abs(self.start_timestamp - target_time))
                first_index = max(0, first_index - 3)  # better for window splitting
        ...
        # if config == "Before_Recognition":
        #     print(
        #         f"Eye Position Z - Min: {-self.eye_position[:, 2].min() - 0.22}, Max: {-self.eye_position[:, 2].max() - 0.22}. difference: {abs(-self.eye_position[:, 2].min() - 0.22 - (-self.eye_position[:, 2].max() - 0.22)) * 100:.1f} cm"
        #     )
    ...
    def get_total_duration(self) -> float:
        # print("start: ", self.start_timestamp[0], "end: ", self.start_timestamp[-1])
